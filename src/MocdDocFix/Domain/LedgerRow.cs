...
public sealed class LedgerRow
{
    ...
    [Index(8), Name("correct service catalogue name")] public string CorrectServiceCatalogueName { get; set; } = string.Empty;

    // ---- the two the operator edits, and what explains them ----

    [Index(9), Name("verdict")] public string Verdict { get; set; } = string.Empty;
    [Index(10), Name("final state")] public string FinalState { get; set; } = string.Empty;
    [Index(11), Name("group")] public int Group { get; set; }

    /// <summary>portal or plugin — which code path created the record, from FileRecordCopier.StyleOf.</summary>
    [Index(12), Name("way of upload")] public string WayOfUpload { get; set; } = string.Empty;

    [Index(13), Name("error")] public string Error { get; set; } = string.Empty;

    /// <summary>Filled once the row has been backed up, so the bytes are one click away.</summary>
    [Index(14), Name("backup path")] public string BackupPath { get; set; } = string.Empty;

    [Index(15), Name("reason of bug")] public string ReasonOfBug { get; set; } = string.Empty;
    [Index(16), Name("solution")] public string Solution { get; set; } = string.Empty;
    [Index(17), Name("notes")] public string Notes { get; set; } = string.Empty;

    // ---- links ----

    [Index(18), Name("crm link of doc")] public string CrmLinkOfDoc { get; set; } = string.Empty;
    [Index(19), Name("crm link of doc file")] public string CrmLinkOfDocFile { get; set; } = string.Empty;

    // ---- ids and machine detail, kept to the right ----

    [Index(20), Name("doc id")] public Guid DocId { get; set; }
    [Index(21), Name("doc file id")] public Guid DocFileId { get; set; }
    [Index(22), Name("service catalogue id")] public string ServiceCatalogueId { get; set; } = string.Empty;
    [Index(23), Name("correct service catalogue id")] public string CorrectServiceCatalogueId { get; set; } = string.Empty;
    ...
    [Index(24), Name("old category")] public string OldCategory { get; set; } = string.Empty;
